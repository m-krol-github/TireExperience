...
export default function TirePath() {
  ...
  const setTemperature = useTime((state) => state.setTemperature);
  ...
  useFrame(() => {
    const t = moveData.current.distance;
    ...
    v.lerpVectors(cameraStartPositon, cameraEndPositon, t);

    cam.current.position.copy(v);
    cam.current.lookAt(pos);

    wheel.current.getWorldPosition(v);

    setTemperature(v.y);
    /*if (phase === "ready") {
      moveData.current.speed = 0;
      moveData.current.distance = 0;
    }*/
  });
